	...
	gamePath: function (gamePath) {
		if (typeof(gamePath) !== 'undefined') {
			if (gamePath.substr(0, 1) === '/') {
				...
				this._gamePath = gamePath.substr(1,gamePath.length) + '/';
			} 
			else if (gamePath.substr(0, 2) === 'cr') {
				this._gamePath = process.cwd() + '/';
			}
			else {
				// Relative path
			...
			if (!this.fs.existsSync(this._gamePath + 'server.js')) {
				...
				this.log('Starting game server in path: ' + this._gamePath);
				return true;
			}
		}
	},
